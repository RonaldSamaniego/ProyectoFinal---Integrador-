...
public class DAOempresadestajero {
 ...
 private Conexion mysql = new Conexion();
    ...
    public DefaultTableModel consultar(String buscar) {
        ...
        sql = "SELECT * FROM empresadestajero WHERE RazonSocial LIKE '%" + buscar + "%'";
        ...
        try {
            ...
            while (rs.next()) {
                registros[0] = rs.getString("IdEmpresaDestajero");
    ...
    public boolean insertar(Eempresadestajero v) {
        if (validarRazonSocialRepetida(v.getRazonSocial())) {
            JOptionPane.showMessageDialog(null, "Ya existe una empresa con esta razón social.", "Error", JOptionPane.ERROR_MESSAGE);
            return false;
        }

        sql = "INSERT INTO empresadestajero (Ruc, RazonSocial, Direccion, Telefono, RepresentanteLegal, IdUsuarioRegistro, Estado) VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    private boolean validarRazonSocialRepetida(String razonSocial) {
        sql = "SELECT COUNT(*) AS total FROM empresadestajero WHERE RazonSocial = ?";
